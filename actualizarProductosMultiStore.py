...
def getColumnNames(tableName):
    ...
    db = pymysql.connect("0.0.0.0","user","pwd","Invoices")
    cursor = db.cursor()
...
if __name__ == '__main__':
    ...
    with open('prodConfMultiStore.json') as json_file:
    ...
    r = requests.get('http://0.0.0.0:5002/productos/v2/{0}'.format(idTienda))
    jsonResponse = r.json()
    ...
    if wentOK == True:
        ...
        if totalReg > 0:
            ...
            resultEnd=requests.get('http://server:5002/productos/actualizacion/v2/{0}/{1}'.format(idTienda,dtRegModificado))
            print('http://server:5002/productos/actualizacion/v2/{0}/{1}'.format(idTienda,dtRegModificado))
            print(resultEnd)
    else:
